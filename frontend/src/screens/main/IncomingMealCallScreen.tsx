...
const VIBRATION_PATTERN = [0, 1000, 500, 1000, 500, 1000, 2000]; // Ring pattern

// ─── Colors ──────────────────────────────────────────────────────────────────
const COLORS = {
  background: ['#0a0a0f', '#111118', '#0d0d12'] as const,
  avatarGradient: ['#10b981', '#059669'] as const,
  acceptButton: ['#22c55e', '#16a34a'] as const,
  declineButton: '#ef4444',
  declineButtonDark: '#dc2626',
...
// ─── Main Component ──────────────────────────────────────────────────────────
export default function IncomingMealCallScreen() {
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const { mealSlotId, autoAccept } = route.params ?? {};
  ...
  useEffect(() => {
    // Staggered ripple rings with smooth easing
    ...
    // Avatar breathing/pulse animation
  ...
  const handleAccept = useCallback(() => {
    ...
    stopRinging();
    navigation.replace('VoiceMealLog', { mealSlotId, autoStart: true });
  }, [mealSlotId, navigation, stopRinging, acceptButtonScale]);
