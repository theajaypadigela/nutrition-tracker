...
const NutritionDetailDrawer: React.FC<NutritionDetailDrawerProps> = ({
  ...
  return (
    <>
        ...
        >
          ...
          <Box className={`w-full p-4 border-b border-${COLORS.bg.tertiary}`}>
            <HStack className="items-center justify-between mb-2">
              <Heading
                size="lg"
                className={`font-bold text-${COLORS.text.primary}`}
              >
              ...
              {(() => {
                ...
                return (
                  <Box className={`px-2.5 py-1 rounded-full bg-${badge.bg}`}>
                    <Text
                      className={`text-[10px] font-semibold text-${badge.text}`}
                    >
                      {badge.label}
                    </Text>
                  </Box>
            ...
            <HStack className="items-center gap-3 mt-1">
              <Text className={`text-[10px] text-${COLORS.text.secondary}`}>
                Current:{' '}
                <Text
                  className={`text-[10px] font-bold text-${COLORS.text.primary}`}
                >
            ...
            <VStack space="md" className="p-4">
              ...
              {nutrientBreakdown.length > 0 &&
                !hasNoTrendData(selectedNutrient.trend) && (
                  <Box>
                    <Text
                      className={`text-xs uppercase tracking-wide font-semibold mb-2 text-${COLORS.text.tertiary}`}
                    >
                      Today's Sources
                    </Text>
                    <VStack space="xs">
                      {nutrientBreakdown.map((food, i) => (
                        ...
                        >
                          <Text
                            className={`text-xs text-${COLORS.text.primary}`}
                          >
                            {food.name}
                          </Text>
                          <HStack className="items-center gap-3">
                            <Text
                              className={`text-xs text-${COLORS.text.secondary}`}
                            >
                              {food.amount}
                              {food.unit}
                            </Text>
                            <Text
                              className={`text-xs font-semibold w-10 text-right text-${COLORS.accent.primary}`}
                            >
                              {food.contribution}%
                            </Text>
                          </HStack>
              ...
              <Box>
                ...
                {hasNoTrendData(selectedNutrient.trend) ? (
                  ...
                  <Box
                    className={`relative rounded-lg p-4 bg-${COLORS.bg.tertiary}`}
                  >
                    ...
                    <VStack className="relative z-10 items-center py-4">
                      <Text
                        className={`text-sm font-semibold mb-1 text-${COLORS.text.primary}`}
                      >
                        No data detected for this nutrient
                      </Text>
                      <Text
                        className={`text-xs mb-4 text-center text-${COLORS.text.secondary}`}
                      >
                        We couldn't detect this nutrient in your last 7 days of
                        call logs.
                      </Text>

                      <HStack className="gap-2 justify-center mb-3">
                        <Pressable
                          className={`px-3 py-1.5 rounded-lg bg-${COLORS.accent.light} flex-row items-center`}
                        >
                          <Icon
                            as={Utensils}
                            size="xs"
                            className={`text-${COLORS.accent.primary} mr-1`}
                          />
                          <Text
                            className={`text-[10px] font-semibold text-${COLORS.accent.primary}`}
                          >
                            Suggest Foods
                          </Text>
                        </Pressable>
                        ...
                        <Pressable
                          className={`px-3 py-1.5 rounded-lg bg-${COLORS.bg.tertiary} flex-row items-center`}
                        >
                          <Icon
                            as={Phone}
                            size="xs"
                            className={`text-${COLORS.text.secondary} mr-1`}
                          />
                          <Text
                            className={`text-[10px] font-semibold text-${COLORS.text.secondary}`}
                          >
                            Recent Calls
                          </Text>
                        </Pressable>
                      ...
                      <HStack className="items-center justify-center gap-1">
                        <Icon
                          as={Info}
                          size="xs"
                          className={`w-[10px] h-[10px] text-${COLORS.text.tertiary}`}
                        />
                        <Text
                          className={`text-[9px] text-${COLORS.text.tertiary}`}
                        >
                  ...
                  <Box>
                    <HStack
                      className="items-end justify-between gap-1 p-2 rounded-lg bg-gray-100"
                      style={{ height: 96 }}
                    >
                      {selectedNutrient.trend.map((val, i) => {
                        const isNoEntry =
                          val === 0 &&
                          hasPartialTrendData(selectedNutrient.trend);
                        const statusKey =
                          val > 100 ? 'high' : val < 50 ? 'low' : 'ok';
                        ...
                        return (
                          ...
                          >
                            <Box
                              className="w-full relative items-center justify-end"
                              style={{ flex: 1 }}
                            >
                              {isNoEntry ? (
                                <Box
                                  className="absolute bottom-0 w-full bg-gray-200 rounded-t items-center justify-center"
                                  style={{ height: '30%' }}
                                >
                                  <Text
              ...
              <HStack className="gap-2 items-stretch">
                ...
                >
                  <Icon
                    as={Pin}
                    size="xs"
                    className={`text-${COLORS.accent.primary}`}
                  />
                  <Text
                    className={`text-xs font-semibold text-${COLORS.accent.primary}`}
                  >
                ...
                >
                  ...
                  <Text
                    className={`text-[9px] font-normal mt-0.5 text-${COLORS.text.tertiary}`}
                  >
      ...
      {showModal && (
        <SetDailyTarget
          RecommendedValue={selectedNutrient.recommendedValue}
          unit={selectedNutrient.unit}
          currentTarget={selectedNutrient.currentTarget}
          showModal={showModal}
